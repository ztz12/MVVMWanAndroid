...
class ScrollAwareFAB2Behavior constructor(context: Context, attrs: AttributeSet) : CoordinatorLayout.Behavior<FloatingActionMenu>(context, attrs) {
    ...
    override fun onNestedPreScroll(coordinatorLayout: CoordinatorLayout, child: FloatingActionMenu, target: View, dx: Int, dy: Int, consumed: IntArray) {
        if (dy > 0) {// 上滑隐藏
            if (child.isOpened) {
                child.close(true)
            }
